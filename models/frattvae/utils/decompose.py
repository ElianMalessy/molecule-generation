...
from .fragmentation import find_BRICSbonds, find_BRICSbonds_and_rings, find_MedChemFrag
...
def HydrogenMatch(f, f_dash, uniquify: bool = True):
    ...
    for order in orders:
        ...
        for a, a_dash in zip(f.GetAtoms(), f_ordered.GetAtoms()):
            if a.GetAtomicNum() != a_dash.GetAtomicNum():
                break
            elif a.GetNumExplicitHs() != a_dash.GetNumExplicitHs():
                break
        else:
            canonOrder = order
            break
...
def MolFromFragments(frag_smiles: list, adj: list, asMol: bool = False, useChiral: bool = True):
    ...
    adj: list shape= (len(frag_smiles), len(frag_smiles)), 0: none, 1: single, 2: double, 3: triple
    """
