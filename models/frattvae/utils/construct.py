...
def constructMol(frag_smiles: list, adj: list, asMol: bool = False, useChiral: bool = True):
    ...
    adj: list or np.ndarray shape= (len(frag_smiles), len(frag_smiles)),
...
def constructMolwithTimeout(frag_smiles: list, adj: list, asMol: bool = False, useChiral: bool = True, timeout: int = 300):
    if _HAS_TIMEOUT:
        ...
        def construct(frag_smiles, adj, asMol, useChiral):
        ...
        try:
            smi = constructMol(frag_smiles, adj, asMol, useChiral)
        except Exception:
            smi = None
    return smi
